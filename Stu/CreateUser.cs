...
LEFT JOIN (
    SELECT StudentId, SkillGroupId, Description, InstructorName
...
WHERE 
    ...
    AND (ISNULL(@LevelStudent, '') = '' OR s.LevelStudent = @LevelStudent)";
        ...
        private void SetGridHeaders()
        {
            ...
            if (dataGridView1.Columns.Contains("Skill1")) dataGridView1.Columns["Skill1"].Visible = false;
            if (dataGridView1.Columns.Contains("Skill2")) dataGridView1.Columns["Skill2"].Visible = false;
            if (dataGridView1.Columns.Contains("Skill3")) dataGridView1.Columns["Skill3"].Visible = false;
        }
            ...
            try
            {
                ...
                Document.Create(container =>
                {
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        string evaluationDate = latestEvaluation != null
                            ? latestEvaluation.EvaluationDate.ToString("yyyy/MM/dd")
                            : "نامشخص";
                        container.Page(page =>
                        {
                            ...
                            page.Content()
                                ...
                                .Column(column =>
                                {
                                    ...
                                    column.Item().PaddingTop(4).Text(description)
...
SELECT 
    ...
    sg2.Name AS Skill2Name, 
    sg3.Name AS Skill3Name,
    (SELECT TOP 1 InstructorName FROM StudentSkillEvaluation 
     WHERE StudentId = s.StudentId AND SkillGroupId = s.Skill1 AND SkillNumber = 1 
     ORDER BY EvaluationDate DESC) AS Instructor1,
    (SELECT TOP 1 InstructorName FROM StudentSkillEvaluation 
     WHERE StudentId = s.StudentId AND SkillGroupId = s.Skill2 AND SkillNumber = 2 
     ORDER BY EvaluationDate DESC) AS Instructor2,
    (SELECT TOP 1 InstructorName FROM StudentSkillEvaluation 
     WHERE StudentId = s.StudentId AND SkillGroupId = s.Skill3 AND SkillNumber = 3 
     ORDER BY EvaluationDate DESC) AS Instructor3
FROM Student s
...
WHERE s.StudentId = @StudentId";
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"خطا در باز کردن فرم ایجاد:\n{ex.Message}", "خطا",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show($"فایل PDF در {filePath} ذخیره شد اما قابل دسترسی نیست لطفاً مسیر را بررسی کنید", "هشدار",
                                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (UnauthorizedAccessException)
            {
                MessageBox.Show("دسترسی به مسیر ذخیره‌سازی وجود ندارد لطفاً دسترسی‌ها را بررسی کنید", "خطا",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (IOException ex)
            {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show($"خطا در باز کردن فرم مربی \n {ex.Message}", "خطا", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
